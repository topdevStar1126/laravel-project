...
class FormProcessor
{
    public function generatorValidation()
    {
        $validation['rules'] = [
            ...
            'form_generator.multi_select.*' => 'nullable|in:0,1',
        ];
        $validation['messages'] = [
            ...
            'form_generator.multi_select.*.in' => 'Some selected select type is invalid',
        ];
        return $validation;
    }
        ...
        if ($forms) {
            for ($i = 0; $i < count($forms['form_label']); $i++) {
                ...
                $formData[$label] = [
                    ...
                    'multi_select' => @$forms['multi_select'][$i] ?? 0,
                ];
            }
        }
        if ($isUpdate) {
    ...
    public function valueValidation($formData)
    {
        ...
        foreach ($formData as $data) {
            ...
            if ($data->type == 'checkbox' || ($data->type == 'select' && $data->multi_select == 1)) {
